...
## 六、其它相关惊喜（非弹窗彩蛋）
- **🍸 Nana 评鉴官（广州探店清单）** `BarTastingList`：首页横向滑动小卡（在 香港/迪士尼 回忆卡下方），列 6 家广州收藏店铺。点小卡 → 高德地图深链 `amap()`（唤起高德 App 弹店铺卡片：地址/照片/评分/导航，免登陆；没装 App 走网页）；点右上圆圈 → 标记「评鉴过」，进度存 `nana-bar-tasted`。改店铺：在 `BarTastingList` 的 `LIST` 数组增删（`{id, e 品类emoji, n 店名, q 高德搜索关键词=店名+区域, area 区域·类型, r 评分}`）。想换回大众点评：把 `amap()` 换成点评搜索深链即可。
