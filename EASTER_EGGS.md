...
|---|---|---|---|
| 🎪 游乐场畅玩 | 行为 | ~~累计玩满 30 次抽键盘券~~ | ⛔ 已下线（2026/7/11 移除所有抽奖彩蛋） |
| 🍽️ 好好吃饭 | 行为 | ~~连续午饭 7 天抽饭盒~~ | ⛔ 已下线（同上；奖励卡包里已有的券仍可兑换） |
...
> 测试：控制台 `localStorage.setItem("nana-fun-plays","29")` 后进游乐场随便点一个馆。
...
- **🎪 游乐场（原游戏室升级）** `FunRoomLobby`：满屏游乐园夜景大厅（摩天轮/木马/彩旗/小卖部/散步猫），三个入口：
  ...
  - **💌 小笺分享**：每张读书小笺右上角点 💌 → 弹「选个信纸样式」（航空/星夜/薄荷/蜜桃，`NANA_LETTER_THEMES`）→ canvas 画成信纸图片 → 系统分享面板发微信/存相册（不支持时自动下载）。改样式/加新款：改 `NANA_LETTER_THEMES` 配色数组即可。
