...
## 六、其它相关惊喜（非弹窗彩蛋）
- **🍸 酒蒙子雷达（广州探店评鉴卡）** `BarTastingList`：首页横向滑动小卡（在 香港/迪士尼 回忆卡下方），列 6 家广州收藏店铺。点小卡 → 高德地图深链 `amap()`（唤起高德 App 弹店铺卡片：地址/照片/评分/导航，免登陆；没装 App 走网页）。
  - **Nana 的评鉴**：每家店有 `v` 态度（`"good"`=👍不错绿卡 / `"bad"`=👎不得行粉卡 / `""`=· 待评鉴灰）和 `note` 一句评语。**「发我我上线」模式**：Nana 把 态度+评语 发我，我填进 `LIST`（改 `v`、`note` 两个字段）提交，所有人打开都能看到。
  - **排序**：没评过的（待评鉴）排前面，评过/写过评语的沉到后面（`reviewed()` + 稳定排序）。占位文案「种草中，还没拔草 🌱」。右上计数「酒了 X/6」= 已评鉴数。⭐ 是大众点评公开分，仅作参考。
  - 改店铺：在 `LIST` 数组增删（`{id, e 品类emoji, n 店名, q 高德搜索关键词=店名+区域, area 区域·类型, r 公开评分, v 态度, note 评语}`）。想换回大众点评跳转：把 `amap()` 换成点评搜索深链即可。
